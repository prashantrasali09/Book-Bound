...
const Book_Card = (props) => {
    ...
    async function add_cart(){
        let response=await axios.get(`http://localhost:8081/add_cart/${_id}/${user._id}`);
    ...
    async function buyop(){
        let response=await axios.post('http://localhost:8081/buy',{_id,user_id:user._id})
    ...
    async function delete_book(){
        console.log(_id)
        let response=await axios.delete(`http://localhost:8081/delete_book/${_id}`);
        makerun(!run)  
    }
